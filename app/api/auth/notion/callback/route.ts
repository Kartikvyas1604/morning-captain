...
export async function GET(request: Request) {
  ...
  const savedState = getOAuthState(sessionId, "notion");
  removeOAuthState(sessionId, "notion");
  ...
  try {
    ...
    const tokens = await exchangeCode(OAUTH_PROVIDERS.notion, code, redirectUri);

    if (tokens.access_token) {
      setToken(sessionId, "notion", JSON.stringify(tokens));
    }
  } catch {
