...
export async function GET(request: Request) {
  ...
  const savedState = getOAuthState(sessionId, "google");
  removeOAuthState(sessionId, "google");
  ...
  try {
    ...
    const tokens = await exchangeCode(OAUTH_PROVIDERS.google, code, redirectUri);

    if (tokens.access_token) {
      const now = Math.floor(Date.now() / 1000);
      tokens.expires_at = String(now + Number(tokens.expires_in || 3600));
      setToken(sessionId, "google", JSON.stringify(tokens));
    }
  } catch {
