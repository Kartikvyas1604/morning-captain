...
export async function GET() {
  const provider = OAUTH_PROVIDERS.slack;
  if (!isProviderConfigured(provider)) {
    return new Response(
      setupPage("slack", "Slack",
        `<ol>
          <li>Go to <a href="https://api.slack.com/apps">Slack API → Your Apps</a></li>
          <li>Create a new App (choose "From scratch")</li>
          <li>Under "OAuth &amp; Permissions", add <code>http://localhost:3000/api/auth/slack/callback</code> as a Redirect URL</li>
          <li>Copy the Client ID and Client Secret from "Basic Information"</li>
        </ol>`
      ),
  ...
  setOAuthState(sessionId, state, "slack");
